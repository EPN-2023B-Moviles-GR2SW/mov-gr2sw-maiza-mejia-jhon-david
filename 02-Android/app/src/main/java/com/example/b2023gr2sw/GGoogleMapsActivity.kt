...
class GGoogleMapsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        solicitarPermisos()
        iniciarLogicaMapa()
        val boton = findViewById<Button>(R.id.btnIrCarolina)
    ...
    fun escucharListeners (){
        ...
        mapa.setOnPolylineClickListener {
            mostrarSnackbar("setOnPolylinegonClickListener ${it.tag}")
            it.tag
        }
    ...
    fun moverQuicentro(){
        ...
        val quicentro = LatLng(
            -0.17675013757339197, -78.47902847058629
        )
    ...
    fun solicitarPermisos(){
        ...
        val permissosFineLocation = ContextCompat
            .checkSelfPermission(
                contexto,
                //permiso que van a checkear
                nombrePermiso
            )
        val tienePermisos = permissosFineLocation == PackageManager.PERMISSION_GRANTED
        if(tienePermisos) {
            permisos = true
        } else {
